...
class CombatRepository:
    ...
    def find_active_encounter_for_entity(self, save_id: str, entity_id: str) -> CombatEncounter | None:
        with session_scope(self.session_factory) as session:
            stmt = (
                select(CombatEncounter)
                .join(Combatant, Combatant.encounter_id == CombatEncounter.id)
                .where(
                    CombatEncounter.save_id == save_id,
                    CombatEncounter.state == EncounterState.ACTIVE,
                    Combatant.entity_id == entity_id,
                )
                .order_by(CombatEncounter.started_at.desc())
            )
