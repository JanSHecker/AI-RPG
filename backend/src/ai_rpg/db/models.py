...
class ScenarioAction(Base):
    ...
    relevant_attribute: Mapped[ActionAttribute] = mapped_column(
        Enum(ActionAttribute, native_enum=False),
        nullable=False,
    )
    difficulty: Mapped[int] = mapped_column(Integer, default=10, nullable=False)
    action_point_cost: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    resolution_mode: Mapped[ActionResolutionMode] = mapped_column(
        Enum(ActionResolutionMode, native_enum=False),
        nullable=False,
    )
    handler_key: Mapped[str | None] = mapped_column(String(120), nullable=True)
    created_by_llm: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
