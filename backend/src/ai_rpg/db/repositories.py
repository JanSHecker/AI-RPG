...
class SaveRepository:
    ...
    def _clone_entity_stats(self, row: EntityStats, save_id: str) -> EntityStats:
        return EntityStats(
            ...
            max_action_points=row.max_action_points,
        )
...
class ScenarioActionRepository:
    def __init__(self, session_factory: sessionmaker[Session]):
        self.session_factory = session_factory

    def list_actions(self, scenario_id: str) -> list[ScenarioAction]:
        with session_scope(self.session_factory) as session:
            stmt = select(ScenarioAction).where(ScenarioAction.scenario_id == scenario_id).order_by(ScenarioAction.name)
            return list(session.scalars(stmt))

    def get(self, action_id: str) -> ScenarioAction | None:
        with session_scope(self.session_factory) as session:
            return session.get(ScenarioAction, action_id)
...
class WorldRepository:
    ...
    def get_save_scenario_id(self, save_id: str) -> str:
        with session_scope(self.session_factory) as session:
            save = session.get(Save, save_id)
            if save is None:
                raise ValueError(f"Unknown save {save_id}")
            return save.scenario_id
    ...
    def resolve_entity_by_name(self, save_id: str, location_entity_id: str, name: str) -> Entity | None:
        ...
        with session_scope(self.session_factory) as session:
            ...
            for entity in session.scalars(stmt):
                role = str((entity.details or {}).get("role", "")).lower()
                if (
                    entity.name.lower() == lowered
                    or lowered in entity.name.lower()
                    or (role and (role == lowered or lowered in role))
                    or entity.name.lower() in lowered
                    or (role and role in lowered)
                ):
                    return entity
            return None
    ...
    def resolve_place_by_name(self, save_id: str, from_place_id: str, name: str) -> Entity | None:
        ...
        with session_scope(self.session_factory) as session:
            ...
            for entity in session.scalars(stmt):
                if entity.name.lower() == lowered or lowered in entity.name.lower() or entity.name.lower() in lowered:
                    return entity
            return None
    ...
    def apply_patch(self, patch: StatePatch) -> None:
        with session_scope(self.session_factory) as session:
            for operation in patch.operations:
                if operation.kind == PatchOperationKind.MOVE_ENTITY:
                ...
                elif operation.kind == PatchOperationKind.ADJUST_ACTION_POINTS:
                    ...
                    if "set_to" in operation.data:
                        next_value = int(operation.data["set_to"])
                    else:
                        next_value = stats.action_points + int(operation.data.get("amount", 0))
                    stats.action_points = max(0, min(stats.max_action_points, next_value))
